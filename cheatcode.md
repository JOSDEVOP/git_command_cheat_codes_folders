...
- -⚡git checkout commit hash_id: it can help you travel back to a particular commit and you can create a branch from there if there is something you need there.This command takes you into detach HEAD and if you want to switch back to ur normal branch the command git switch - or git switch main can be used.In the git checkout commit hash id if we can to make a copy of a particular branch we use the code git switch -c name of branch to copy the current copy.
